    ...
    public static class IPAddressExtensions
    {
        ...
        public static string GetReverseLookupAddress(this IPAddress ipAddress)
        {
            ...
            else
            {
                for (int i = addressBytes.Length - 1; i >= 0; i--)
                {
                    string hex = addressBytes[i].ToString("x2", CultureInfo.InvariantCulture);
                    res.Append(hex[1]);
                    res.Append(".");
                    res.Append(hex[0]);
                    res.Append(".");
        ...
        public static DomainName GetReverseLookupDomain(this IPAddress ipAddress)
        {
            ...
            if (ipAddress.AddressFamily == AddressFamily.InterNetwork)
            {
                ...
                for (int i = addressBytes.Length - 1; i >= 0; i--)
                {
                    labels[labelPos++] = addressBytes[i].ToString(CultureInfo.InvariantCulture);
                }
            ...
            else
            {
                ...
                for (int i = addressBytes.Length - 1; i >= 0; i--)
                {
                    ...
                    labels[labelPos++] = hex[1].ToString(CultureInfo.InvariantCulture);
                    labels[labelPos++] = hex[0].ToString(CultureInfo.InvariantCulture);
                }
