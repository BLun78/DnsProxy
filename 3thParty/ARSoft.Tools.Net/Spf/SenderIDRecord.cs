    ...
    public class SenderIDRecord : SpfRecordBase
    {
        ...
        public override string ToString()
        {
            ...
            else
            {
                ...
                res.Append("/");
                res.Append(string.Join(",", Scopes.Where(s => s != SenderIDScope.Unknown).Select(s => EnumHelper<SenderIDScope>.ToString(s).ToLower(CultureInfo.InvariantCulture))));
            }
        ...
        private static bool TryParsePrefix(string prefix, out int version, out int minor, out List<SenderIDScope> scopes)
        {
            ...
            version = Int32.Parse(match.Groups["version"].Value, CultureInfo.InvariantCulture);
            minor = Int32.Parse("0" + match.Groups["minor"].Value, CultureInfo.InvariantCulture);
